...
def exchangeBuy(c,p):
    ...
    Rate = soup.find_all(class_='rate-content-cash text-right print_hide')
    if c=='美金':
        return '新台幣'+str(p*float(Rate[1].text.strip()))+'圓';
    elif c=='日圓':
        return '新台幣'+str(p*float(Rate[15].text.strip()))+'圓';
    elif c=='人民幣':
        return'新台幣'+str(p*float(Rate[37].text.strip()))+'圓';
    elif c=='歐元':
        return  '新台幣'+str(p*float(Rate[29].text.strip()))+'圓';
    elif c=='英鎊':
        return '新台幣'+str(p*float(Rate[5].text.strip()))+'圓';
    else:
        return '請確認輸入資訊'
